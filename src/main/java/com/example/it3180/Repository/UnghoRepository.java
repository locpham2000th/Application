...
public interface UnghoRepository extends JpaRepository<UnghoEntity, Long> {
    ...
    @Query(value = "select u from UnghoEntity u where u.donggop.id like %?1%")
    UnghoEntity findUngHo(String idDongGop);

    @Query(value = "select count(u.id) from UnghoEntity u left join DonggopEntity d on d.id = u.donggop  where d.id= ?1")
    Long countHoGiaDinh(String id);

    @Query(value = "select sum(u.sotien) from UnghoEntity u left join DonggopEntity d on d.id = u.donggop where d.id= ?1 ")
